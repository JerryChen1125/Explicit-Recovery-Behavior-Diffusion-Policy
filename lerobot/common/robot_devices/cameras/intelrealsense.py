...
class IntelRealSenseCamera:
    ...
    def connect(self):
        ...
        if self.use_depth:
            if self.fps and self.width and self.height:
                if self.serial_number[0] == 'f':
                    config.enable_stream(rs.stream.depth, 1024, 768, rs.format.z16, self.fps)
                else:
    ...
    def read(self, temporary_color: str | None = None) -> np.ndarray | tuple[np.ndarray, np.ndarray]:
        ...
        frame = self.camera.wait_for_frames(timeout_ms=50000)
        try:
            frame = self.align.process(frame)
            # print("Aligned")
        except Exception as e:
            print(f"Error during align process: {str(e)}")
        ...
        if self.use_depth:
            ...
            h, w = depth_map.shape
            if self.serial_number[0]!= 'f' and (h != self.height or w != self.width):
                raise OSError(
