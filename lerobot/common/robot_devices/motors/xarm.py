...
class XarmBus:
    ...
    def __init__(
        ...
        self.gripper = "gripper" in config.motors
        self.mock = None
    ...
    def read(self, data_name, motor_names: str | list[str] | None = None):
        ...
        if data_name == "Present_Position":
            if self.gripper:
                ...
                code, positive=self.robot.get_position()
                # pos_value=np.zeros(7)
                values = servo_angle
                pos_value=positive
               
                # pos_value[-1]=gripper_pos
                values[-1] = gripper_pos
            else:
                code, servo_angle = self.robot.get_servo_angle()
                values = servo_angle[:6]
                pos_value=positive[:6]
            # TODO 
            # print(servo_angle)
